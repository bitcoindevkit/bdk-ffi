...
impl Client {
    pub async fn next_log(&self) -> Option<Log> {
        let mut log_rx = self.log_rx.lock().await;
        log_rx.recv().await.map(|log| log.into())
    }

    pub async fn next_warning(&self) -> Option<Warning> {
        let mut warn_rx = self.warning_rx.lock().await;
        warn_rx.recv().await.map(|warn| warn.into())
    }
